...
cbs_df <- read_xls(here("data/kwb-2019.xls", na = "."))

# Load police data. Note that missings are ".".
burglary_df <- read_delim(here("data/Misdrijven_per_wijk_en_buurt_per_jaar_03122021_095311.csv"), trim_ws = TRUE)
...
wijk_sf <- st_read(here("data/WijkBuurtkaart_2019_v3/wijk_2019_v3.shp"), quiet = TRUE)
...
# Arrange.
main_plot <- plot_grid(map_gg, scat_gg, ncol = 2, scale = c(1,0.9)) +
...
# Add NSCR logo.
nscr_logo <- png::readPNG(here::here("static/img/logos/nscr_logo_snow.png"))
...
ggsave(filename = (here::here("static/img/burglary/burglary_ams.svg")), height = 29.7, width = 42, unit = "cm")
